Gui, Add, Button, gOK x132 y259 w190 h30 , OK
Gui, Add, Button, gCancel x332 y259 w190 h30 , Cancel
Gui, Add, Button, gDelete x530 y259 w90 h30 , Delete
Gui, Add, Edit, x33 y29 w590 h170 , 1. Export links of all extensions using `n https://chrome.google.com/webstore/detail/export-links-of-all-exten/cmeckkgeamghjhkepejgjockldoblhcb `n2.Save the results under file:///D:/Downloads/extensions.html`n3.Run the script, click "ok", let it do its job ( it will use mouse input and keyboard input so better leave the computer as a whole )`n4.In D:\Downloads\Test3.txt you can find a list of extensions that could not be installed `n5.You can delete extensions using the delete button and going to the brave://extensions/ page. `n6. Configured to work with Brave, but can work with any Chrome based browser
; Gui, Add, Edit, x32 y209 w590 h20 +HScroll, ; 
; Generated using SmartGUI Creator 4.0
Gui, Show, x127 y127 h314 w658, New GUI Window
Return
...
OK:
FileSelectFile, SelectedFile,,extensions.html, 
...
FileRead, OutputVar, %SelectedFile%
{
    
    GoodPart := StringBetween( OutputVar, "<ol class=""enabled"">", "</ol>" )

    ; GoodPart := StringBetween( A_LoopReadLine, "<li><a href=""", """ target=" )

}

; MsgBox, %GoodPart%
FileDelete, D:/Downloads/Test2.txt
FileAppend, %GoodPart%, D:/Downloads/Test2.txt
Loop, read, D:/Downloads/Test2.txt 
{
    ; MsgBox % A_LoopReadLine
    GoodLine := StringBetween( A_LoopReadLine, "<li><a href=""", """ target=" )
    if(GoodLine != "")
    {
    ; Array[A_Index] := GoodLine
    ; Array%A_Index% := %GoodLine%
    Array.Push(GoodLine) 
    EndIndex := EndIndex + 1
    } 
...
{
    ; MsgBox % A_LoopReadLine
    GoodLine := StringBetween( A_LoopReadLine, "target=""_blank"" title=", "</a></li>" )
    
    if(GoodLine != "")
    {
    ; Array2[A_Index] := GoodLine
    ; Array2%A_Index% := %GoodLine%
    Array2.Push(GoodLine) 
    EndIndex2 := EndIndex2 + 1
    } 
    ; MsgBox % Array[A_Index]
}

FileDelete, D:/Downloads/Test3.txt
MaxLength := % MaxLength(Array)
; MaxLength - 74 ; EndIndex - 100
; %EndIndex%

CoordMode, Mouse, Relative
loop %EndIndex% ; Enumeration is the recommended approach in most cases.
{
    if (BreakLoop = 1)
    {
        break
    }
    ; MsgBox, "element " %element%. "index " %index%. "Array%index1% " Array[%index%]. "Array%index2% " Array2[%index%]

    msgbBoxVar1 := % Array[A_Index]
    msgbBoxVar2 := % Array2[A_Index]
    msgbBoxVar2Array := StrSplit(msgbBoxVar2 , """>")
    msgbBoxVar2ArrayString := % msgbBoxVar2Array[2]
    ; MsgBox, "msgbBoxVar1 " %msgbBoxVar1% "msgbBoxVar2 " %msgbBoxVar2ArrayString%

    VarToShow := % MaxLength(Array) 
    SplashTextOn, 400, 300,, %A_Index% / %EndIndex%
    settimer, TurnOffTheSplash, -600

    Sleep, 500
    WinActivate, ahk_exe Brave.exe
    Sleep, 200
    Run, brave.exe %msgbBoxVar1%
    Sleep, 3500
    Click, 1370, 215 
    Sleep, 1500
    SendInput, {left}
    ...
    Text:="|<>*147$28.33k60QTUs1la3UD6MS0gEXM6l29Un49a3yNbwDtaTk37s6AAD0Ms"
    ...
    ; . "Pos:`t[677, 365]  " X ", " Y "`n`n"
    ; . "Result:`t" (ok ? SuccessFunction(msgbBoxVar2ArrayString) Comment : FailedFunction())

    if (ok)
    {
      SuccessFunction(msgbBoxVar2ArrayString)
    }

    ; MsgBox, % ok.MaxIndex()
    ; MsgBox, %ok%
    ;  % ok.MaxIndex(), % (ok ? SuccessFunction(msgbBoxVar2ArrayString) Comment : FailedFunction())

    SuccessFunction(msgbBoxVar2ArrayString)
    {
    ; MsgBox, " FAILED FFFFFFFFFFFFF          ".%msgbBoxVar2ArrayString%              ; "Failed !"
    FileAppend, %msgbBoxVar2ArrayString%`n, D:/Downloads/Test3.txt
    return
    }
