...
BarChart(ChartData, Width, Height="", TitleText="", Skin="LightOrange", Options="") {	; by Learning one
	...
	if Width is not number
	{
		...
		if (SubStr(Width,1,4) = "hwnd") {	; extra feature for AHK_L users - use  handle of the control
			...
			if PotentialHwnd is number
				...
				GuiControlGet, hControl, %GuiNumFixed%:hwnd, %ControlID%
		}
		else		
			GuiControlGet, hControl, %GuiNumFixed%:hwnd, %ControlID%
			
		GuiControlGet, Control, %GuiNumFixed%:Pos, %ControlID%	; get control's Width & Height
	...
	if SetBitmap2Control
	{
		...
		GuiControl, %GuiNumFixed%:MoveDraw, %ControlID%	; repaints the region of the GUI window occupied by the control
