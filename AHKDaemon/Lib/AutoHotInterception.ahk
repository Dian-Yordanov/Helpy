...
class AutoHotInterception {
	...
	GetDeviceId(IsMouse, VID, PID, instance := 1) {
		...
		if (dev == 0) {
			MsgBox, 4,, % "Could not get " devType[isMouse] " with VID " VID ", PID " PID ", Instance " instance ", !!! This error was changed to throw an error so it can be picked by AHK. Press 'YES' to use normal AHK based code and 'NO' to exit because subscription based mode is not available. "
			IfMsgBox No
				; MsgBox, "ggg"
				throw A_ThisFunc "Error_thrown"
			IfMsgBox Yes
				ExitApp
    		return
			throw A_ThisFunc "Error_thrown"
