...
class AutoHotInterception {
	...
	GetDeviceId(IsMouse, VID, PID, instance := 1) {
		...
		if (dev == 0) {
			MsgBox, 1, % "Could not get " devType[isMouse] " with VID " VID ", PID " PID ", Instance " instance
			ExitApp
		}
		return dev
	}
	...
	GetDeviceIdFromHandle(isMouse, handle, instance := 1) {
		...
		if (dev == 0) {
			MsgBox , 1, "Could not get " devType[isMouse] " with Handle " handle ", Instance " instance
			ExitApp
		}
		return dev
	}
